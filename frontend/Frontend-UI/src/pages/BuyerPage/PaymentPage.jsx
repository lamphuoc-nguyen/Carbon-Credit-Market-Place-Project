...
const PaymentPage = () => {
    ...
    const formatPrice = (price) => {
        // 🔑 FIX: Cung cấp giá trị mặc định là 0 nếu price là null/undefined
        const validPrice = price === null || price === undefined ? 0 : price;

        // Đảm bảo validPrice là một số trước khi gọi .toFixed
        if (typeof validPrice !== 'number' && typeof validPrice !== 'string') {
            return 'N/A';
        }

        // Chuyển chuỗi thành số nếu cần, và xử lý định dạng
    ...
    // 🔑 FIX: Sử dụng _ để tránh lỗi ESLint 'no-unused-vars'
    const [_, setSelectedPendingTransaction] = useState(null);
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            if (paymentMethod === 'vnpay') {
                ...
                const result = await buyerApi.initiatePurchaseTransaction(
                    listing.id,
                    'VNPAY_ONLINE', // methodId (string)
                ...
                if (!result.transactionId || !result.paymentUrl) {
                    // Kiểm tra xem lỗi có phải do Backend trả về WALLET không
                    if (result.paymentMethod === 'WALLET') {
                        throw new Error('Payment method mismatch: Expected VNPAY but received WALLET transaction. Check API parameters and backend service logic.');
                    }
                        ...
                        {new Date(tx.createdAt).toLocaleString()}
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center">
                                            <div className="flex-1 mb-3 sm:mb-0">
                                                ...
                                                <div className="text-sm text-gray-700">
                                                    <span className="font-semibold">Total Price:</span> ${formatPrice(tx.totalPrice || tx.amount || 0)}
                                                </div>
                                                <div className="text-sm text-gray-700">
