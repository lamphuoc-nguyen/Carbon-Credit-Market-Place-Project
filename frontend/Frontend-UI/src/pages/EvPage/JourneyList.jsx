      ...
      reader.onload = async (e) => {
        try {
          ...
          try {
            ...
            const testResponse = await EvOwnerAPI.journeys.getMyJourneys()
            console.log('✅ Backend is reachable:', testResponse.status)
          } catch (error) {
            console.error('❌ Backend connectivity test failed:', error)
            setUploadStatus('error')
            setUploadMessage('Backend is not reachable. Please check if the server is running.')
            return
          }
          ...
          for (let i = 0; i < rows.length; i++) {
            try {
              console.log(`🚀 Sending journey ${i + 1}:`, rows[i]);
              
              // Clean and validate the data before sending
              const journeyData = {
                journeyId: rows[i].journeyId || `journey_${Date.now()}_${i}`,
                distanceKm: parseFloat(rows[i].distanceKm),
                startTime: rows[i].startTime,
                endTime: rows[i].endTime,
                vehicleId: rows[i].vehicleId,
                co2ReducedKg: parseFloat(rows[i].co2ReducedKg),
                energyConsumedKwh: parseFloat(rows[i].energyConsumedKwh),
                verificationStatus: rows[i].verificationStatus || 'PENDING',
                verificationNotes: rows[i].verificationNotes || '',
                rejectionReason: rows[i].rejectionReason || '',
                userId: rows[i].userId,
                verifiedById: rows[i].verifiedById || null,
                createdAt: rows[i].createdAt || new Date().toISOString(),
                verificationDate: rows[i].verificationDate || null
              };
          
              // Add a quick check for the required energy field
              if (!journeyData.energyConsumedKwh || journeyData.energyConsumedKwh <= 0) {
                  throw new Error("Validation failed: Energy consumption must be a positive number.");
              }
          
              console.log(`📤 Cleaned journey data:`, journeyData);
          
              // Pass the CORRECT, complete data to the API
              const response = await EvOwnerAPI.journeys.createJourney(journeyData); // ✅ CORRECTED
              console.log(`✅ Journey ${i + 1} created successfully:`, response.data);
              
              successCount++;
              
              setUploadProgress(50 + (i / rows.length) * 40);
          ...
          if (errorCount === 0) {
            ...
            await fetchJourneys() // Refresh the journey list
          } else {
            setUploadStatus('error')
            setUploadMessage(`Uploaded ${successCount} journeys, ${errorCount} failed. Errors: ${errors.slice(0, 3).join(', ')}${errors.length > 3 ? '...' : ''}`)
          }
              ...
              {uploadStatus && (
                <div className="mb-4">
                  <div className={`p-3 rounded-lg flex items-center gap-2 ${
                    ...
                    {uploadStatus === 'uploading' && <AlertCircle size={20} />}
